...
SPAGxE_Plus = function(GenoFile = NULL,
                       GenoFileIndex = NULL,
...

{
  ...
  sparseGRM = obj.SPAGxE_Plus_Nullmodel$sparseGRM   # update on 2024-12
  ...
  if(pval.norm1 > epsilon){
    ...
    # S_GxE0.var = (GRM1 %>% data.table::dplyr::select(Cov_R.new0_sqrtMAF) %>% sum) * 2 - sum(R.new0^2 * g.var.est)
    ...
    if(abs(z_GxE0) < Cutoff){
      ...
      # update
      S_GxE0.mean.new = S_GxE0.mean * sqrt(Var.ratio.S_GxE0)

      pval1 = GetProb_SPA_G_new(init.t = 0, MAF, R.new0, (S_GxE0.mean + abs(S_GxE0.new - S_GxE0.mean.new)), lower.tail = FALSE)  # SPA-G p value
      pval2 = GetProb_SPA_G_new(init.t = 0, MAF, R.new0, (S_GxE0.mean - abs(S_GxE0.new - S_GxE0.mean.new)), lower.tail = TRUE)  # SPA-G p value
...
#' @param gIDs a character vector of subject IDs. NOTE: its order should be the same as the subjects order of the Geno.mtx (i.e. the input of the function SPAGxE()).
...

{
  ...
  ####
  R_GRM_R = sum(GRM1$Cov_R) * 2 - sum(R^2)
  ...
  ####
  RE_GRM_RE = sum(GRM1$Cov_RE) * 2 - sum((RE)^2)

  ####
  R_GRM_RE = sum(GRM1$Cov_R_RE_part1) +
    sum(GRM1$Cov_R_RE_part2) -
    sum(RE * R)
  ...
  ####
  R.new_GRM_R.new = sum(GRM1$Cov_R.new) * 2 - sum(R.new^2)
  ...
  obj.SPAGxE_Plus_Nullmodel = list(ResidMat = ResidMat,
                                   ...
                                   sparseGRM = sparseGRM)                          # update on 2024-12
...
SPAGxE_Plus_Nullmodel_resid = function(data=NULL,
                                       pIDs=NULL,
                                       gIDs=NULL,
                                       range=c(-100,100),
                                       length.out = 10000,
