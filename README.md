...
```
Current version is 1.0.0. For older version and version update information, plesase refer to OldVersions.  
...
To analyze individuals from multi-ancestries or multi-way admixed populations with family relateness, SPAGxEmix<sub>CCT</sub>+ uses PC-AiR to obtain PCs representing population structure. Then, SPAGxEmix<sub>CCT</sub>+ uses PC-Relate to regress out the PCs from the genotypes to get ancestry-adjusted genotypes, and calculate kinship to estimate family structure (representing recent genetic relateness. In PC-Relate paper, it has been demonstrated that an iterative procedure alternating between PC-AiR and PC-Relate can potentially provide improved inference on both population structure (with PC-AiR) and recent genetic relatedness (with PC-Relate). Generally two iterations of PC-AiR and PC-Relate is sufficient, and accurate PCs ancestry-adjusted sparse GRM can be ontained. Step 1 and 2 are similar to SPAGxEmix<sub>CCT</sub>. Note that in step 2, ancestry-adjusted sparse GRM is needed to charactrize family structure, and SPA is also used to calibrate p-values.

**SPAGxEmix<sub>CCT</sub>+ is a scalable and accurate analytical G×E framework which can control for both population structure and family relateness for any population structures and family structures, even for cohorts containing individuals from multi-ancestry or multi-way admixed populations with ancestry-specific MAFs and ancestry-specific case-control ratios!**
