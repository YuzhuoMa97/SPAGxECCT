...
SPAGxE+ is a scalable and accurate G×E analytical framework that uses saddlepoint approximation to calibrate the distribution of test statistics while controlling for sample relatedness and case-control imbalance. Compared to SPAGxE, the sparse genetic relationship matrix (GRM) is used for characterizing familial structure. SPAGxE+ provides accurate p-values even when case-control ratios are extremely unbalanced. Additionally, SPAGxE+ is applicable to other complex traits including time-to-event, ordinal categorical, and longitudinal traits and maintains highly accurate even when phenotypic is unbalanced.
